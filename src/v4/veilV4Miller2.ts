...
/** Optimized-v4 Miller slice 3 of 4 (23 prepared lines). */
export class VeilV4Miller2 extends SmartContract {
    @prop(true)
    stateHash: Sha256
    ...
    @prop()
    readonly gammaLines: FixedArray<PreparedLine, 23>

    @prop()
    readonly deltaLines: FixedArray<PreparedLine, 23>

    constructor(
        stateHash: Sha256,
        nextCodeHash: Sha256,
        gammaLines: FixedArray<PreparedLine, 23>,
        deltaLines: FixedArray<PreparedLine, 23>
    ) {
        super(...arguments)
    ...
    public advance(snapshot: V4MillerState, nextCodePart: ByteString) {
        ...
        // Global Miller digit 47: 0
        acc = BN256.squareFQ12(acc)
        line = BN256Pairing.lineFuncDouble(r, snapshot.p0)
        acc = BN256Pairing.mulLine(acc, line.a, line.b, line.c)
        acc = StagedGroth16.mulPreparedLine(acc, this.gammaLines[22], snapshot.p1)
        acc = StagedGroth16.mulPreparedLine(acc, this.deltaLines[22], snapshot.p2)
        r = BN256.modTwistPoint(line.rOut)
        acc = BN256.modFQ12(acc)
