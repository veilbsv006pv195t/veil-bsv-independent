...
template ShieldedPool(DEPTH) {
    ...
    // V2: knowing the note plaintext is not authorization to spend it.
    // Only the recipient knows the preimage of the public owner identifier.
    component spendingOwner = Hash2();
    // Fixed public plaintext, SECRET MiMC key. Reversing these inputs would
    // expose an invertible permutation under a publicly known key.
    spendingOwner.left <== 1447381298;
    spendingOwner.right <== inputSpendingKey;
    spends * (spendingOwner.out - inputOwnerKey) === 0;
    component spendingKeyZero = IsZero();
    spendingKeyZero.in <== inputSpendingKey;
    spends * spendingKeyZero.out === 0;
    inputSpendingKey * (1 - spends) === 0;
